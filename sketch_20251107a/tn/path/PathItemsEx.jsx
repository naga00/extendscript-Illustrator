(function() {
    ...
    PathItemsEx.initialize = function() {
        ...
        PathItemsEx.prototype.drawTriangleEx = function(x0, y0, x1, y1, x2, y2, x3, y3) {
            this.moveTo(x0, y0);
            this.lineTo(x1, y1);
            this.lineTo(x2, y2);
            this.lineTo(x3, y3);
        ...
        PathItemsEx.prototype.drawSmoothCurvedLine = function(points, vertex) {
            ...
            this.moveCurveTo(points[0].x, points[0].y);
            var i = 1;
            while (i < vertex) {
                this.curveTo(points[i - 1].x, points[i - 1].y, (points[i].x + points[i - 1].x) / 2, (points[i].y + points[i - 1].y) / 2);
                i++;
            }
        };
        ...
        PathItemsEx.prototype.drawSmoothCurvedRing = function(points, vertex) {
            ...
            this.moveTo((points[0].x + points[pointsLength - 1].x) / 2, (points[0].y + points[pointsLength - 1].y) / 2);
            var i = 0;
            while (i < vertex - 1) {
                this.curveTo(points[i].x, points[i].y, (points[i].x + points[i + 1].x) / 2, (points[i].y + points[i + 1].y) / 2);
                i++;
            }
            this.curveTo(points[pointsLength - 1].x, points[pointsLength - 1].y, (points[pointsLength - 1].x + points[0].x) / 2, (points[pointsLength - 1].y + points[0].y) / 2);
        };
        ...
        PathItemsEx.prototype.drawGear = function(x, y, side, innerRadius, outerRadius, angle, holeSides, holeRadius) {
            ...
            this.moveTo(x + (Math.cos(startAngle) * outerRadius), y - (Math.sin(startAngle) * outerRadius));
            ...
            if (holeSides > 2) {
                ...
                this.moveTo(x + (Math.cos(startAngle) * holeRadius), y - (Math.sin(startAngle) * holeRadius));
